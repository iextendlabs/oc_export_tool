	...
	protected function getList() {
		...
		if ($api_info && $this->user->hasPermission('modify', 'sale/order')) {
			...
			$data['api_token'] = '';
				...
				$data['href_export'] = $this->url->link('sale/order/export', 'user_token=' . $this->session->data['user_token'] .$url, true);
	...
	public function invoice() {
		...
		$this->response->setOutput($this->load->view('sale/order_invoice', $data));
				...
				public function export(){
					...
					$header = array('Order Id','Customer Name','Order Status','Total Amount','Email','Payment Method','Shipping City','Date Added','Date Modified');

					fputcsv($file,$header);

					$results = $this->model_sale_order->getAllOrders();
					foreach ($results as $result) {
						
						$export_data = array();
						$export_data[]= $result['order_id'];
						$export_data[]= $result['customer_name'];
						$export_data[]= $result['order_status'];
						$export_data[]= $result['total_amount'];
						$export_data[]= $result['email'];
						$export_data[]= $result['payment_method'];
						$export_data[]= $result['shipping_city'];
